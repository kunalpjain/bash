...
void parsecommand(char *completepath,char *path,char **argv,int nofargs) { //indexes all the symbols
	// fprintf(stdout, "Path is %s and nofargs is %d\n", path,nofargs);
	for(int i=0;i<nofargs;i++){
		if(argv[i]==NULL) {
			break;
		}
	...
	for(int i=0;i<=countofpipe;i++) {
		...
		if(fork()==0) {
			...
			if(i!=countofpipe) {
				...
				for(int q=0;q<index-prev;q++)
					printf("%s ", newcommand[q]);
				printf("\n");
				dup2(p[i][1],1);
			...
			prev = index+1;
			if(i!=countofpipe-1)
				index = strinarr(argv+prev,"|",nofargs-prev);
			else
				index = nofargs;
			FILE *fpout = fopen("outlog.txt","a");
			fprintf(fpout, "%d %d %d\n", i,prev,index-1);
